...
void random_matrix_size_generate(uint32_t max_macs, uint32_t max_mac_size, int32_t* A_rows, int32_t* comm_dim, int32_t* B_cols){
    int A, B, Comm_dimm;
    A = random_number_generate(64);
    *A_rows = A < 2 ? ((A * -1) < 2 ? 2 : A * -1): A;
    uint32_t max_B_cols = (uint32_t) ((float)max_macs / (float)*A_rows);
    B = random_number_generate(max_B_cols);
    *B_cols = (uint32_t)(B < 2 ? (((B * -1) < 2) ? 2 : B * -1): B);
    Comm_dimm = (int32_t) random_number_generate(max_mac_size);
    *comm_dim = Comm_dimm < 2 ? ((Comm_dimm * -1) < 2 ? 2 : Comm_dimm * -1): Comm_dimm;
}
...
void write_to_csv_error(char* string, float* error_array, int rangeA, int rangeB) {
    ...
    fprintf(file, "%.6f,%.6f,%.6f,%i,%i\n", error_array[0],error_array[1],error_array[2], rangeA, rangeB);
    
    // Close the file
    fclose(file);
}
void write_to_csv_latency(char* string, double* latency_array, int rangeA, int rangeB) {
    // Open the CSV file in append mode
    FILE *file = fopen(string, "a");
    if (file == NULL) {
        // File doesn't exist, create it
    ...
    // Write the values to the file
    fprintf(file, "%.f,%.f,%.f,%.f,%.f,%.f,%.f,%.f,%.f,%d,%d\n", latency_array[0],latency_array[1],latency_array[2], latency_array[3],latency_array[4],latency_array[5],latency_array[6],latency_array[7],latency_array[8],rangeA,rangeB);
    ...
    for (int j=MIN_RANGE;j<=MAX_RANGE;j++){
        for (int i=MIN_RANGE;i<=MAX_RANGE; i++){
            ...
            for(int k=0;k<MATRIX_TEST;k++){
                random_matrix_size_generate(128, MAX_MAC_SIZE-1, &A_rows, &comm_dim, &B_cols);
                //printf("A_rows, comm_dim, B_cols, rangeA, rangeB: %d, %d, %d, %d, %d\n", A_rows, comm_dim, B_cols,i,j);
                ...
                //printf("Software_matrix\n");
                //float_matrix_print(&software_result,';');
                double latency_SW;
                double latency_HW_CARGAR;
                double latency_HW_WAIT;
                double latency_HW_LEER;
                latency_SW = (double) ((end_SW.tv_sec - start_SW.tv_sec)*1000000000 + (end_SW.tv_nsec - start_SW.tv_nsec));;
